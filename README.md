...
Note: you will need to also potentially also checkmark off Spoof enabled in the web interface so it will emulate the now missing DS1302 chip in the future i will remove this redundancy as there is no way to keep the DS1302 in place that i have found. Also the alarm and time set buttons will not function anymore and by default this clock will run 24 hour time now. Also i recommend you remove the coin cell battery from the white YM-SZ010-L V0.5 board for safety there is no need to keep it. Also I would recommend a tiny heatsink on the ESP32-C3 SuperMini as I have found at least with my boards that they can get a bit hot. As for how to attach it for me i used vhb tape to the white plastic bottom near where the light is being careful to keep the usb c port easy to access should i need to program the ESP32-C3 SuperMini again for any reason or for a future update though my location means each time I need to program i need to open the case again. I do not at the time have a good solution for making the programming usb available outside the case.
